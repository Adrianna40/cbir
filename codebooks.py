...
# %%
# let's test time complexity of numpy and scipy cosine similarity 
import time 

# first scipy 
start = time.time()
for i in range(len(visual_words)):
    cos = distance.cosine(visual_words[0], visual_words[i])
end = time.time()
print('scipy', end-start)

# then our function with numpy 
start = time.time()
for i in range(len(visual_words)):
    cos = cosine_similarity(visual_words[0], visual_words[i])
end = time.time()
...
def generate_codebook(features_by_category : dict, centers : np.array) -> dict:
    """ 
    Takes 2 parameters: 
    ...
    for category, features in features_by_category.items():
        ...
        for img_features in features:
            ...
            image_bovw = np.zeros(len(centers))
...
save_to_json('codebook_test0.json', codebook_test)
save_to_json('codebook_train0.json', codebook_train)
